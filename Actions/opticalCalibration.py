...
class OpticalCalibration(MakesmithInitFuncs):
    ...
    def reloadCalibration(self):
        try:
            xyErrors = self.data.config.getValue(
                "Optical Calibration Settings", "xyErrorArray"
            )
            self.calErrorsX, self.calErrorsY = self.data.config.parseErrorArray(
                xyErrors, True
            )
            self.xCurve[0] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX0")
            )
            self.xCurve[1] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX1")
            )
            self.xCurve[2] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX2")
            )
            self.xCurve[3] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX3")
            )
            self.xCurve[4] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX4")
            )
            self.xCurve[5] = float(
                self.data.config.getValue("Optical Calibration Settings", "calX5")
            )
            self.yCurve[0] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY0")
            )
            self.yCurve[1] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY1")
            )
            self.yCurve[2] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY2")
            )
            self.yCurve[3] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY3")
            )
            self.yCurve[4] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY4")
            )
            self.yCurve[5] = float(
                self.data.config.getValue("Optical Calibration Settings", "calY5")
            )
    ...
    def setCalibrationSettings(self, args):
        ...
        self.opticalCenter = (
            float(args["opticalCenterX"]),
            float(args["opticalCenterY"]),
        )
    ...
    def saveOpticalCalibrationConfiguration(self, args):
        ...
        try:
            ...
            self.data.config.setValue(
                "Optical Calibration Settings", "scaleX", float(args["scaleX"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "scaleY", float(args["scaleY"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings",
                "gaussianBlurValue",
            ...
            self.data.config.setValue(
                "Optical Calibration Settings",
                "cannyLowValue",
                float(args["cannyLowValue"]),
            )
            self.data.config.setValue(
                "Optical Calibration Settings",
                "cannyHighValue",
                float(args["cannyHighValue"]),
            )
            self.data.config.setValue(
                "Optical Calibration Settings",
                "autoScanDirection",
            ...
            self.data.config.setValue(
                "Optical Calibration Settings", "markerX", float(args["markerX"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "markerY", float(args["markerY"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "tlX", int(args["tlX"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "tlY", int(args["tlY"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "brX", int(args["brX"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "brY", int(args["brY"])
            )
            self.data.config.setValue(
                "Optical Calibration Settings",
                "calibrationExtents",
                args["calibrationExtents"],
            )
            self.data.config.setValue(
                "Optical Calibration Settings",
                "positionTolerance",
                args["positionTolerance"],
            )
        except Exception as e:
            self.data.console_queue.put(str(e))
            return False
    ...
    def processImage(self, findCenter=False):
        ...
        while True:
            ...
            if cv2.contourArea(c) > 1000:
                ...
                xD = self.distance((tlblX, tlblY), (trbrX, trbrY)) / (
                    self.markerX * 25.4
                )
                yD = self.distance((tltrX, tltrY), (blbrX, blbrY)) / (
                    self.markerY * 25.4
                )
        ...
        self.data.console_queue.put(
            "Got 10 images processed, " + str(falseCounter) + " images were bad"
        )
        if falseCounter != 10:
    ...
    def on_CenterOnSquare(self, _dist, findCenter=False):
        ...
        if avgDx is not None:
            cv2.putText(
                image,
                "(" + str(self.HomingPosX) + ", " + str(self.HomingPosY) + ")",
                (15, 15),
            ...
            cv2.putText(
                image,
                "Dx:{:.3f}, Dy:{:.3f}->Di:{:.3f}mm".format(avgDx, avgDy, avgDi),
                (15, 40),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                (0, 0, 255),
            ...
            cv2.line(
                image, (int(xA), int(yA) - 15), (int(xA), int(yA) + 15), (255, 0, 0), 1
            )
            cv2.line(
                image, (int(xA) - 15, int(yA)), (int(xA) + 15, int(yA)), (255, 0, 0), 1
            )
            ...
            self.HomingY += avgDy
            if (
                (abs(avgDx) >= self.positionTolerance)
                or (abs(avgDy) >= self.positionTolerance)
            ) and (not findCenter):
            ...
            if self.inAutoMode == False:
                ...
                if self.autoScanDirection == 0:  # horizontal
                    ...
                    if self.HomingPosY >= maxY:
                        self.HomingY -= (
                            7.0  # drop down 7 mm for next square's guess (only)
                        )
                        self.HomeIn()
                    else:
                        self.inAutoMode = False
                        ...
                        data = {
                            "errorX": self.calErrorsX.tolist(),
                            "errorY": self.calErrorsY.tolist(),
                        }
    ...
    def findCenter(self, args):
        ...
        if avgxB is not None and avgyB is not None:
            ...
            # )
            self.data.ui_queue1.put(
                "Action", "updateOpticalCalibrationFindCenter", data
            )
            return True
        return False
    ...
    def saveCalibrationToCSV(self):
        try:
            ...
            dateTime = datetime.datetime.fromtimestamp(currentTime).strftime(
                "%Y%m%d-%H%M%S"
            )
    ...
    def saveAndSend(self):
        try:
            ...
            self.data.config.setValue(
                "Optical Calibration Settings", "calX0", str(self.xCurve[0])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calX1", str(self.xCurve[1])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calX2", str(self.xCurve[2])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calX3", str(self.xCurve[3])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calX4", str(self.xCurve[4])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calX5", str(self.xCurve[5])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY0", str(self.yCurve[0])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY1", str(self.yCurve[1])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY2", str(self.yCurve[2])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY3", str(self.yCurve[3])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY4", str(self.yCurve[4])
            )
            self.data.config.setValue(
                "Optical Calibration Settings", "calY5", str(self.yCurve[5])
            )

            self.data.config.setValue(
                "Optical Calibration Settings", "xyErrorArray", _str
            )
            return True
        except Exception as e:
            self.data.console_queue.put(str(e))
    ...
    def surfaceFit(self):
        ...
        try:
            ...
            xA = np.c_[
                np.ones(dataX.shape[0]),
                dataX[:, :2],
                np.prod(dataX[:, :2], axis=1),
                dataX[:, :2] ** 2,
            ]
            ...
            yA = np.c_[
                np.ones(dataY.shape[0]),
                dataY[:, :2],
                np.prod(dataY[:, :2], axis=1),
                dataY[:, :2] ** 2,
            ]
