...
class Kinematics(MakesmithInitFuncs):
    ...
    h = math.sqrt(
        (l / 2) * (l / 2) + s * s
    )  # distance between sled attach point and bit
    ...
    def triangularInverse(self, xTarget, yTarget):
        ...
        Motor1Distance = math.sqrt(
            math.pow((-1 * self._xCordOfMotor - xTarget), 2)
            + math.pow((self._yCordOfMotor - yTarget), 2)
        )
        Motor2Distance = math.sqrt(
            math.pow((self._xCordOfMotor - xTarget), 2)
            + math.pow((self._yCordOfMotor - yTarget), 2)
        )
        ...
        if self.chainOverSprocket == 1:
            Chain1Angle = math.asin(
                (self._yCordOfMotor - yTarget) / Motor1Distance
            ) + math.asin(self.R / Motor1Distance)
            Chain2Angle = math.asin(
                (self._yCordOfMotor - yTarget) / Motor2Distance
            ) + math.asin(self.R / Motor2Distance)
        ...
        TotalWeight = self.sledWeight + 0.5 * self.chainDensity * (
            Chain1Straight + Chain2Straight
        )
        ...
        Tension1 = (
            -(
                TotalWeight
                * math.sqrt(
                    math.pow(xTangent1 - xTarget, 2) + math.pow(yTangent1 - yTarget, 2)
                )
                * (xTangent2 - xTarget)
            )
            / TensionDenominator
        ...
        Tension2 = (
            TotalWeight
            * math.sqrt(
                math.pow(xTangent2 - xTarget, 2) + math.pow(yTangent2 - yTarget, 2)
            )
            * (xTangent1 - xTarget)
        ) / TensionDenominator
        ...
        # Catenary Equation
        Chain1 = math.sqrt(
            math.pow(2 * a1 * math.sinh((xTarget - xTangent1) / (2 * a1)), 2)
            + math.pow(yTangent1 - yTarget, 2)
        )
        Chain2 = math.sqrt(
            math.pow(2 * a2 * math.sinh((xTangent2 - xTarget) / (2 * a2)), 2)
            + math.pow(yTangent2 - yTarget, 2)
        )
        ...
        Chain1 = Chain1AroundSprocket + Chain1 / (
            1.0 + self.leftChainTolerance / 100.0
        ) / (1 + Tension1 * self.chainElasticity)
        Chain2 = Chain2AroundSprocket + Chain2 / (
            1.0 + self.rightChainTolerance / 100.0
    ...
    def quadrilateralInverse(self, xTarget, yTarget):
        ...
        if (
            ...
            self.x = (
                self.D - self.x
            )  # Chain lengths are swapped at exit if the x,y is on the right half
            self.Mirror = True
        ...
        self.Crit[1] = -self._YOffsetEqn(
            self.Y1Plus, self.x - self.h * self.CosPsi1, self.SinPsi1
        )
        self.Crit[2] = -self._YOffsetEqn(
            self.Y2Plus, self.D - (self.x + self.h * self.CosPsi2), self.SinPsi2
        )
        ...
        while Tries <= self.MaxTries:
            ...
            self.Jac[3] = (
                self._YOffsetEqn(
                    self.Y1Plus, self.x - self.h * self.CosPsi1D, self.SinPsi1D
                )
                + self.Crit[1]
            ) / self.DeltaPhi
            self.Jac[4] = (
            ...
            self.Jac[6] = (
                self._YOffsetEqn(
                    self.Y2Plus,
                    self.D - (self.x + self.h * self.CosPsi2D),
                    self.SinPsi2D,
                )
                + self.Crit[2]
            ) / self.DeltaPhi
            ...
            self.Y1Plus = (
                self.Y1Plus + self.Solution[1]
            )  # don't allow the anchor points to be inside a sprocket
            if self.Y1Plus < self.R:
                self.Y1Plus = self.R

            self.Y2Plus = (
                self.Y2Plus + self.Solution[2]
            )  # don't allow the anchor points to be inside a sprocke
        ...
        self.TanGamma = (self.y - self.Offsety1 + self.Y1Plus) / (
            self.x - self.Offsetx1
        )
        self.TanLambda = (self.y - self.Offsety2 + self.Y2Plus) / (
            self.D - (self.x + self.Offsetx2)
        )
        ...
        if self.Mirror:
            Chain2 = (
                math.sqrt(
                    ...
                    + (self.y + self.Y1Plus - self.Offsety1)
                    * (self.y + self.Y1Plus - self.Offsety1)
                )
    ...
    def forward(self, chainALength, chainBLength, xGuess=-10, yGuess=10):
        ...
        while 1:
            ...
            if (
                abs(aChainError) < 0.000000001 and abs(bChainError) < 0.000000001
            ) or guessCount > 5000:
                if guessCount > 5000:
                    print
    ...
    def _moment(
        ...
        self.TanLambda = (self.y - self.Offsety2 + Y2Plus) / (
            self.D - (self.x + self.Offsetx2)
        )
    ...
    def _MyTrig(self):
        ...
        self.SinPsi1 = (
            -0.0942 * Psi1cu - 0.1368 * Psi1sq + 1.0965 * self.Psi1 - 0.0241
        )  # sinPsi1
        self.CosPsi1 = (
            0.1369 * Psi1cu - 0.6799 * Psi1sq + 0.1077 * self.Psi1 + 0.9756
        )  # cosPsi1
        self.SinPsi2 = (
            -0.1460 * Psi2cu - 0.0197 * Psi2sq + 1.0068 * self.Psi2 - 0.0008
        )  # sinPsi2
        self.CosPsi2 = (
            0.0792 * Psi2cu - 0.5559 * Psi2sq + 0.0171 * self.Psi2 + 0.9981
        )  # cosPsi2

        self.SinPsi1D = (
            -0.0942 * Psi1delcu - 0.1368 * Psi1delsq + 1.0965 * Psi1del - 0.0241
        )  # sinPsi1
        self.CosPsi1D = (
            0.1369 * Psi1delcu - 0.6799 * Psi1delsq + 0.1077 * Psi1del + 0.9756
        )  # cosPsi1
        self.SinPsi2D = (
            -0.1460 * Psi2delcu - 0.0197 * Psi2delsq + 1.0068 * Psi2del - 0.0008
        )  # sinPsi2
        self.CosPsi2D = (
            0.0792 * Psi2delcu - 0.5559 * Psi2delsq + 0.0171 * Psi2del + 0.9981
        )  # cosPsi2
    ...
    def initializeSettings(self):
        ...
        self.R = (
            float(self.data.config.getValue("Computed Settings", "distPerRot"))
            / 3.141592
            / 2
        )
        self.machineHeight = float(
            self.data.config.getValue("Maslow Settings", "bedHeight")
        )
        self.machineWidth = float(
            self.data.config.getValue("Maslow Settings", "bedWidth")
        )
        self.motorOffsetY = float(
            self.data.config.getValue("Maslow Settings", "motorOffsetY")
        )
        self.leftChainTolerance = float(
            self.data.config.getValue("Advanced Settings", "leftChainTolerance")
        )
        self.rightChainTolerance = float(
            self.data.config.getValue("Advanced Settings", "rightChainTolerance")
        )
        self.chainElasticity = float(
            self.data.config.getValue("Advanced Settings", "chainElasticity")
        )
        self.sledWeight = float(
            self.data.config.getValue("Maslow Settings", "sledWeight")
        )
        self.rotationDiskRadius = float(
            self.data.config.getValue("Advanced Settings", "rotationRadius")
        )
        self.chainSagCorrection = float(
            self.data.config.getValue("Advanced Settings", "chainSagCorrection")
        )
        self.chainOverSprocket = int(
            self.data.config.getValue("Computed Settings", "chainOverSprocketComputed")
        )
        kinematicsType = int(
            self.data.config.getValue("Computed Settings", "kinematicsTypeComputed")
        )
        if kinematicsType == 1:
            self.isQuadKinematics = True
        else:
